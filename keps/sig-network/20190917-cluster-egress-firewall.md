...
attempts to block egress traffic:
  ...
  - In the common case of trying to block traffic to ports on nodes or
    masters, the obvious ways of implementing the feature are not
    sufficient in some clusters.
...
packets (eg, proxies or load balancers) must ensure that they cannot
be used to bypass egress firewall rules.
...
connecting to the "ssh" port on nodes. A simple approach to doing this
would be to add an iptables firewalling rule like:

    # "172.17.0.0/16" is the pod network. "10.0.1.0/24" is the subnet
    # the nodes are on
    iptables -A INPUT -s 172.17.0.0/16 -d 10.0.1.0/24 -p tcp -m tcp --dport 22 -j REJECT

But there are numerous problems with this approach:

  - An attacker could create a Service of type LoadBalancer, with its
    Endpoints pointing at a node IP, and then connect to that Service
    from the pod; after passing through the cloud load balancer, the
    connection would no longer appear to be coming from a pod, and so
    would not match the firewalling rule. (With some network plugins,
    it is possible to do this just with an ordinary ClusterIP
    Service.)

  - An attacker that is able to create a secondary [network
    attachment] in a pod could route traffic to the node through that
    interface rather than its default network interface, and connect
    to the node's IP via that unrecognized source IP.

  - An attacker could connect to an alternate node IP that is not
    blocked by the firewalling rule. Eg, assuming that the example
    above has pods attached to `docker0`, a pod would be able to ssh
    to its node by connecting to `172.17.0.1`. Nodes may also have
    additional public IP addresses in some cases, either created by
    the administrator (eg, a second network interface attached to a
    backend network), or created by some third-party Kubernetes
    component as part of its own functionality.

  - (And of course, if the cluster later expands to include nodes
    whose IPs aren't included in the original firewalling rules, the
    administrator needs to remember to update the rules on every node
    to restrict traffic to the new node addresses as well.)
...
[network attachment]: https://docs.google.com/document/d/1Ny03h6IDVy_e_vmElOqR7UdTPAG_RNydhVE1Kx54kFQ/edit

### Goals

- An administrator should be able to prevent all (non-privileged,
  non-hostNetwork) pods in a cluster from being able to access
  specified destinations outside the cluster.

- An administrator should be able to prevent all (non-privileged,
  non-hostNetwork) pods in a cluster from being able to connect to
  specified ports on nodes or masters, and the rules created to
  implement this will automatically update as the set of nodes and
  masters change, and will cover all IP addresses on the node/master.
  (That is, there should be special syntax for saying that you want to
  block access to nodes/masters, without the administrator needing to
  enumerate node/master IPs.)
...
- An administrator should be able to prevent users from creating
  externally-accessible Services (eg, ExternalIP, LoadBalancer) that
  point to restricted destinations in or around the cluster.

- It should be possible to have multiple API objects specifying
...
- Being able to (fully) block privileged pods. In some cases,
  Kubernetes can put traffic-blocking rules in places that are
  inaccessible to pods (eg, in the root network namespace). But in
...
- DNS-based rules. eg, "block access to everything except
  ...
  well.
    ...
    - A DNS-based "deny" rule (eg, "don't allow connections to
      ...
      addresses that we don't know about. On the other hand, DNS-based
      "allow" rules on top of a larger "deny" substrate are secure (as
      long as a single IP address is not hosting both sites you want
      to allow and sites you want to block).

- Per-Namespace firewall rules. While there are certainly use cases
  for this, it makes things substantially more complicated:

    - To reliably prevent pods from accessing IPs, it is necessary to
      prevent the creation of Services pointing to those IPs, but if
      some namespaces can access an IP and others can't, then you need
      to trust that the users in unblocked namespaces won't create
      Services pointing to the IPs that are blocked from other
      namespaces.

    - It may be difficult or impossible to track the source namespace
      of a connection all the way until it leaves the cluster. In
      particular, it may be impossible to track it through a cloud
      load balancer.

    - (Although, in both of the above situations, since Kubernetes
      knows where the Service / LoadBalancer is pointing to, it could
      just block the connection *before* it hits the Service /
      LoadBalancer IP rather than after.)

    - Per-Node restrictions might be simpler to implement than
      per-Namespace, and can solve some of the same problems (eg,
      allowing pods on "infrastructure" nodes to access destinations
      that pods on ordinary nodes cannot).

  Since a namespaced firewall object would have to be a separate
  resource type from the non-namespaced cluster-wide firewall objects
  anyway, this functionality could probably be added in later
  (although that would require changes in every component that
  implements any part of the firewall spec).
...
OpenShift 4.1 and 4.2 because there is no way to disable the built-in
blocking.
...
In OpenShift 4.x, new nodes in cloud environments can be automatically
provisioned, but this implies that if a pod can pretend to be an
unprovisioned node (eg, by using a secondary network attachment to get
an unused IP address on the node network) then it can (several steps
down the line) get the master to send it cluster-admin credentials.
This means that [pods need to be blocked from talking to the Machine
Config Server](https://github.com/openshift/origin/pull/22821).
OpenShift would want to include a policy enforcing this as part of the
default cluster setup.

Similarly, in ovn-kubernetes, nodes need to be able to provide
information about the cluster configuration to the OVN databases, but
[pods need to be prevented from doing
so](https://github.com/ovn-org/ovn-kubernetes/pull/790). The
ovn-kubernetes project could write a `ClusterEgressFirewall` resource
blocking access to this server, and include it for the administrator
to create along with the other objects that must be created in order
to run ovn-kubernetes as a plugin (`ServiceAccount`, `Deployment`,
etc).
...
2. The Service proxy needs to write rules that reject connections to
   services that point to blocked IPs. (We can't just have an
   admission controller forbidding the creation of Endpoints pointing
   to blocked IPs, because the set of blocked IPs can change over
   time.)

3. Any CloudProviders / Ingress / etc that work with Endpoints
...
working together, they need easily-composable semantics. (In
particular, we can't allow both arbitrary allow rules and arbitrary
deny rules in any object, since then the final result would depend on
what order the rules were processed in.)
...
sites, an administrator may want "block everything except ..." or
"allow everything except ..."

However, the user stories above all involve wanting to block specific
sites, with rules that are independent of each other. This implies we
can't just do default deny plus allow rules as with NetworkPolicy.

OTOH, if we implemented DNS-based rules, then as discussed above, they
only make sense as "allow" rules on top of a larger "deny".
